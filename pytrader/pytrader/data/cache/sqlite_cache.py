...
def _default_cache_db_path() -> str:
    """
    PyPSXService uses this SQLite file for HTTP response caching.

    Default beside this module breaks when ``pytrader_sdk`` is bind-mounted from the host
    with root-owned files while the API runs as non-root (typical docker-compose prod).
    """
